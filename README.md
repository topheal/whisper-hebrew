...
folders:
  ...
  # תיקייה עם סיכום אוטומטי בנוסף ל-SRT, בלי הגבלת גיל קבצים, ויצירת קובץ אודיו אם יש רק וידאו
...
`extract_audio: true` - אם הקובץ שנמצא הוא וידאו בלבד (אין קובץ אודיו תואם), נוצר גם קובץ `<שם>.mp3` באמצעות ffmpeg, לנוחות האזנה בלי לפתוח את הוידאו. ברירת המחדל היא `false` (לא נוצר קובץ אודיו).
